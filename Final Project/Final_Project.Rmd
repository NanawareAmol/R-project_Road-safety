...
The following scatter plot show the total test pass count for each test centre from the year 2013 till year 2018 excluding 2014 due to incomplete data. The questions that can be answered by this graph are, <br/>
...
<div style = "display: inline-block;width: 45%;padding-left: 15px;margin-bottom: 80px;">
...
p1 <- ggplotly(ggplot(data = filter(passfail1,Year != '2014'), aes(x = Centre, y = Totalpass, color = Year,  size = TotalPass1)) + geom_point(alpha = 0.5) + 
...
```
<!--<img src = "1.jpg" style = "margin-left: 60px;"><img src = "1.jpg">-->
...
p2 <- ggplotly(ggplot(data = filter(passfail1,Year != '2014'), aes(x = Centre, y = totPassPercentage, color = Year,  size = totPassPercentage1)) + geom_point(alpha = 0.5) + 
...
p <- plot_ly(passfail, x = passfail$Year, y = passfail$Totalpass, color = ~passfail$Year, type = "box", text = paste("Centre = ", passfail$Centre)) %>%
  layout(title = "Yearly performance", yaxis = list(title = 'Total Pass Count'), xaxis = list(title = 'Year'), font = t, legend = list(x = 0.92, y = 0.98, bgcolor = "transparent"))
p
```
</div>
