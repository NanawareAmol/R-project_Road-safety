...
<div style = "display: inline-block;width: 50%;padding-left: 15px;margin-bottom: 80px;">
...
```{r,echo=FALSE, fig.show="hold", fig.width=5, fig.height=4}
p1
```
</div>
...
p <- plot_ly(passfail, x = passfail$Year, y = passfail$Totalpass, color = ~passfail$Year, type = "box", text = paste("Centre = ", passfail$Centre)) %>%
  layout(title = "Yearly performance", yaxis = list(title = 'Total Pass Count'), xaxis = list(title = 'Year'), font = t, legend = list(x = 0.9, y = 0.98))
p
```
</div>
